...
var noop = function(){}
var HotTracks = require('./hottracks');
...
Room.prototype.next = function() {
  ...
  redis.lpop(queueKey, function(err, queueItemStr) {
    if (err) {
      console.error("Error popping queue:", err.stack);
      return;
    }
    if (queueItemStr) self.play(JSON.parse(queueItemStr));
    self.autoFillQueue();
  });
}
...
Room.prototype.autoFillQueue = function() {
  ...
  function queueHotTrack(err, trackStr) {
    if (err) {
      console.error("Error getting hot track:", err.stack);
      return;
    }
    self.enqueueBy(null, JSON.parse(trackStr), function(err) {
      if (err) {
        console.error("Error queueing hot track:", err.stack);
        return;
      }
    });
  }
